...
test_that("comorbidity works no matter what the name of the id variable is", {
  ...
  tgt <- comorbidity(x = x, id = "id", code = "code", score = "charlson", assign0 = "score", tidy.codes = TRUE)
  colnames(x) <- c("testid", "code")
  obj <- comorbidity(x = x, id = "testid", code = "code", score = "charlson", assign0 = "score", tidy.codes = TRUE)
  ...
  rdm <- comorbidity(x = x, id = rdmname, code = "code", score = "charlson", assign0 = "score", tidy.codes = TRUE)
...
test_that("comorbidity works no matter what the name of the code variable is", {
  ...
  obj <- comorbidity(x = x, id = "id", code = "diag", score = "charlson", assign0 = "score", tidy.codes = TRUE)
  expect_equal(object = obj, expected = tgt, check.attributes = FALSE)
  # with a randomly generated name:
  rdmname <- paste(sample(x = LETTERS, size = 20, replace = TRUE), collapse = "")
  colnames(x) <- c("id", rdmname)
  rdm <- comorbidity(x = x, id = "id", code = rdmname, score = "charlson", assign0 = "score", tidy.codes = TRUE)
  expect_equal(object = rdm, expected = tgt, check.attributes = FALSE)
})
