...
.labelled <- function(x, map) {
  attr(x, "variable.labels") <- if (grepl("^charlson_", map) && grepl("cdmf$", map)) {
    # CDMF Charlson labels
    c("ID", "Myocardial infarction", "Congestive heart failure", "Peripheral vascular disease", "Cerebrovascular disease", "Dementia", "Chronic pulmonary disease", "Rheumatoid disease", "Peptic ulcer disease", "Mild liver disease", "Diabetes without chronic complications", "Diabetes with chronic complications", "Hemiplegia or paraplegia", "Mild or moderate renal disease", "Cancer (any malignancy)", "Moderate or severe liver disease", "Severe renal disease", "Metastatic solid tumour", "HIV infection, no AIDS", "AIDS (HIV infection with opportunistic infection)")
