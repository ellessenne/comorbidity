...
# Install comorbidity
devtools::install_github(repo = "ellessenne/comorbidity", ref = "#13")
...
done <- tibble::enframe(list.files(path = "/scratch/cvdanalysis/ag475/comorbidity-results/", pattern = "^res-13-")) %>%
...
for (i in seq(nrow(left))) {
  ...
  saveRDS(object = out, file = paste0("/scratch/cvdanalysis/ag475/comorbidity-results/res-13-", left$i[i], ".RDS"))
  rm(out)
}
