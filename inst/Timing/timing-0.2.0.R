...
# Install comorbidity 0.2.0
devtools::install_github("ellessenne/comorbidity", ref = "0.2.0")
...
done <- tibble::as.tibble(list.files(path = "/scratch/cvdanalysis/ag475/comorbidity-results/", pattern = "^res-0.2.0-")) %>%
...
for (i in seq(nrow(left))) {
  ...
  tstart <- Sys.time()
  cmb <- comorbidity::comorbidity(x = df, id = "id", code = "code", score = "charlson")
  tend <- Sys.time()
  rm(df, cmb)
  gc()
  out <- difftime(tend, tstart, units = "secs")
  saveRDS(object = out, file = paste0("/scratch/cvdanalysis/ag475/comorbidity-results/res-0.2.0-", left$i[i], "-", left$j[i], ".RDS"))
  rm(out)
}
