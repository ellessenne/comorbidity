...
Simulating ICD-10 codes for AHRQ Elixhauser requires simulating DRG codes and ICD-10 ranks. The `comorbidity` packages includes a function named `sample_drg()` that allows simulating DRG codes in a straightforward way. For example:

```{r simulate-data-ahrq}
x = data.frame(
  id = sample(1:5, size = 50, replace = TRUE),
  code = sample_diag(n = 50),
  icd_rank = 1
...
Alternatively, Elixhauser scores based on AHRQ's SAS program (version 3.7) can be calculated provided that data includes DRG codes and ICD-10 code ranks:

```{r elixhauser-ahrq-2020}
x = data.frame(
  id = sample(1:5, size = 50, replace = TRUE),
  code = sample_diag(n = 50),
  icd_rank = 1
...
elixhauser_ahrq_2020 = comorbidity(x = x,
                                   id = 'id', 
                                   code = 'code', 
...
Elixhauser scores Refined for ICD-10-CM based on AHRQ's SAS program (v2021.1) can also be calculated provided that data includes ICD-10 code ranks, present on admission (POA) status codes and either admission calendar year and quarter:

```{r elixhauser-ahrq-2021}
x = data.frame(
  id = sample(1:5, size = 50, replace = TRUE),
  code = sample_diag(n = 50),
  poa = sample(c('Y', 'W', 'N', 'U'), 50, replace=TRUE),
  icd_rank = 1
)
x <- x[order(x$id, x$code), ]
x$icd_rank =  ave(x$icd_rank, x$id, FUN=cumsum) # Simulate ICD Rank
x[c('year', 'quarter')] = sample_year_quarter(5)[x$id,] # Get year/quarter

elixhauser_2021 <- comorbidity(
        x = x, 
        id = 'id', 
        code = 'code', 
...
The AHRQ Elixhauser comorbidities are calculated using code from AHRQ's SAS program version 3.7 (Healthcare Cost and Utilization Project (HCUP)).
...
* Healthcare Cost and Utilization Project (HCUP). (2017). _HCUP Elixhauser Comorbidity Software (3.7)_. Agency for Healthcare Research and Quality, Rockville, MD. [www.hcup-us.ahrq.gov/toolssoftware/comorbidity/comorbidity.jsp](www.hcup-us.ahrq.gov/toolssoftware/comorbidity/comorbidity.jsp)
