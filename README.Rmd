...
The data fields of the data set are:<br /> 
- DischargeFiscalYearNBR (Year of Discharge)<br />
- QuarterNBR (Year Quarter)<br />
- MSDRG (MS-DRG)<br />
- ICD10DiagnosisCD (ICD-10 Code)<br />
- ICD10DiagnosisSEQ (Sequence of ICD-10 Code)<br />
- PresentOnAdmissionCD (POA)

```{r ahrq_section_test_data}

x10 <- as.data.frame(readRDS("data//ahrq_section_test_data.rds"))
print(x10 %>% dplyr::filter(ID %in% c(1:3)), row.names = FALSE)
```
...
Comorbidity codes for different versions of the AHRQ’s ‘Elixhauser Comorbidity Software’ could be computed. Accordingly, 'map' argument could take any of the following ones:<br />
- elixhauser_ahrq_2020<br />
- elixhauser_ahrq_2021<br />
- elixhauser_ahrq_2022<br />
The following example calculates the comorbidity codes for the 2022 version. 

```{r ahrq}
elixhauser_ahrq_2022 <- comorbidity(
x=x10 %>% dplyr::filter(ID %in% c(1:3)),
id='ID',
...
elixhauser_ahrq_2022 <- elixhauser_ahrq_2022 %>% rename(PatientAccountID = ID)
